...
all_data <- all_data %>%
  mutate(
    side_of_ball = case_when(
      position.x %in% c("QB", "RB", "FB", "WR", "TE", "OL", "C", "G", "T") ~ "Offense",
      position.x %in% c("DE", "DT", "LB", "OLB", "ILB", "MLB", "CB", "DB", "FS", "SS", "NT", "SAF", "EDGE") ~ "Defense",
      position.x %in% c("K", "P", "LS") ~ "Special Teams",
      TRUE ~ "Unknown"
    )
  )
...
## Team work 
- Lyle was responsible for data cleaning and  created three visualizations: a bubble plot, bar graph, and scatter plot, as well as a summary table.
